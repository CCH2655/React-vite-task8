...
import {
  createAsyncDelAdminOrders,
  createAsyncGetAdminOrders,
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black bg-opacity-50 backdrop-blur-sm">
      <div className="bg-white rounded-lg shadow-xl w-full max-w-md p-6">
        ...
        <p className="text-gray-500 mb-6">
          ...
          <br />
          編號：
          <span className="font-mono font-bold text-red-600 break-all">
            {orderId}
          </span>
        </p>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black bg-opacity-50 backdrop-blur-sm">
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-3xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-6 space-y-8">
          <section>
            <h4 className="text-sm font-semibold text-blue-600 uppercase mb-3">
              客戶資訊
            </h4>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 bg-gray-50 p-4 rounded-lg">
              ...
              <div>
                ...
                <p className="font-medium">{order.user?.tel}</p>
              </div>
              <div>
...
function AdminOrders() {
  ...
  const fetchOrders = async (page = 1) => {
    setIsLoading(true);
    try {
      await dispatch(createAsyncGetAdminOrders(page)).unwrap();
    } catch (err) {
      showError(err || "讀取訂單失敗");
    } finally {
      // 延遲讓動畫更平滑，不閃爍
  ...
  useEffect(() => {
    // dispatch(createAsyncGetAdminOrders());
    fetchOrders();
  }, []);
        ...
        <div className="p-6">
          ...
          {isLoading ? (
            <div className="flex flex-col items-center justify-center py-40">
              <RotatingLines
                strokeColor="#2563eb" // 使用藍色符合管理員色調
                strokeWidth="3"
                animationDuration="0.75"
                width="50"
                visible={true}
              />
              <p className="mt-4 text-sm font-medium text-gray-400 tracking-widest">
                正在讀取訂單資料...
              </p>
            </div>
          ) : (
            <>
                    ...
                    {orders?.map((order) => (
                      ...
                      >
                        ...
                        <td className="px-6 py-3">
                          <span
                            className={`px-2.5 py-0.5 rounded-full text-xs font-bold ${order.is_paid ? "bg-green-100 text-green-700" : "bg-orange-100 text-orange-700"}`}
                          >
                        ...
                        <td className="px-6 py-3 space-x-2">
                          <button
                            className="text-white bg-blue-500 hover:bg-blue-600 px-3 py-1 rounded shadow-sm text-xs"
                            onClick={() => openView(order)}
                          >
                            查看
                          </button>
                          <button
                            className="text-red-600 border border-red-200 hover:bg-red-50 px-3 py-1 rounded text-xs"
                            onClick={() => openDelConfirm(order)}
                          >
                            刪除
                          </button>
                        </td>
              ...
              <div className="mt-8 flex justify-center">
                <Pagination
                  ...
                  changePage={(page) =>
                    dispatch(createAsyncGetAdminOrders(page))
                  }
                />
              </div>
            </>
